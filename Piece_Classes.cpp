...
void Piece_Classes::piece::printLocation() {
	...
	std::cout << (char)(((location - 1) % 8 + 1) + 96) << (int)((location - 1) / 8 + 1) << std::endl;
}
...
void Piece_Classes::piece::moveTo(unsigned char location) {
	//std::cout << "moves a piece to a specifed location, does not check if move is valid\n";
}
bool Piece_Classes::piece::isSpaceOccupied() {  //TODO: (maybe useful for getLegalMove) finish definition; will check if a space is occupied
	return false;
}
